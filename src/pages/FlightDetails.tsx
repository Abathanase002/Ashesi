...
const FlightDetails = () => {
  ...
  const customerId = currentUser?.userType === 'Customer' ? (currentUser as Customer).id : undefined;
  const { data: userBookings = [], isLoading: isLoadingBookings } = useQuery<Booking[], Error>({
    ...
    // Enable only when auth is not loading and user is authenticated customer
  ...
  // Show loading skeleton if flight data, bookings data, OR auth state is loading
  ...
  const handleBookFlight = () => {
    // Check authentication status ONLY after auth loading is complete
    if (isAuthLoading) return; // Do nothing if auth is still loading
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      ...
                      <Button
                        ...
                        // Disable button if auth is loading, already booked, or booking is pending
                      ...
                      <div className="text-center text-sm text-gray-500">
                        {/* Show login message only if auth is loaded and user is not authenticated */}
                        {!isAuthLoading && !isAuthenticated && "You'll need to login to complete your booking"}
                      </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {isBookingConfirmed ? (
            <div className="py-6">
              ...
              <Button className="w-full" onClick={goToBookings}>
                View My Bookings
              </Button>
            </div>
          ) : (
            <div className="py-6">
              ...
              <p className="mb-4 text-sm text-gray-600">
                You are about to book the flight from {flight.DepartureLocationData?.Name} to {flight.ArrivalLocationData?.Name}.
              </p>
